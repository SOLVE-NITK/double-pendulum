...
function varchange() {
  ...
  $("#fSlider").slider({ max: 0.13, min: 0.05, step: 0.001 });

  // number initialisation : jQuery widget
  $("#fSpinner").spinner({ max: 0.13, min: 0.05, step: 0.001 });
  ...
  $("#omegaSlider").slider({ max: 0.13, min: 0.05, step: 0.001 });

  // number initialisation : jQuery widget
  $("#omegaSpinner").spinner({ max: 0.13, min: 0.05, step: 0.001 });
  // monitoring change in value and connecting slider and number
  // setting trace point coordinate arrays to empty on change of link length
...
document.querySelector(".btn-side").addEventListener("click", openSidebar);
